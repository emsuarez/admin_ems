  ...
  return (
    <div>
      <div onClick={() => setCreate(true)} className=' w-fit mb-10 -mt-10'>
        <h3
          className='mt-3 bg-blue-900 w-32 text-center font-semibold rounded-md
                 text-white hover:cursor-pointer hover:bg-blue-800'
        >
          Crear Ejecutivo
        </h3>
      </div>
                      ...
                      {columns.map(column => {
                        return (
                          <TableCell key={column.id} align={column.align}>
                            ...
                            {[column.id] == 'Ejectivos' && data.nombres}
                            {[column.id] == 'Alias' && data.alias}
                            {[column.id] == 'Creado' && data.created}
                          </TableCell>
                        )
                      })}
                    </TableRow>
