...
interface Column {
  id:
    | 'Centralista de operaciones saliente'
    | 'Fetch Salida'
    | 'Centralista de operaciones entrante'
    | 'Fetcha Llegada'
    | 'Entrega y recepcion de turno';
  label: string;
  minWidth?: number;
  align?: 'right';
...
const columns: Column[] = [
  {
    id: 'saliente',
    label: 'Centralista de operaciones saliente',
    minWidth: 100,
  },
  { id: 'Salida', label: 'Fetch Salida', minWidth: 100 },
  {
    id: 'entrante',
    label: 'Centralista de operaciones entrante',
    minWidth: 150,
  },
...
export default function HistorialTable() {
  ...
  return (
    <div>
        ...
        <div>
          <div className='pb-4 z-50 flex hover:cursor-pointer'>
            <div className='flex space-x-8'>
              ...
              <p onClick={() => setOpenDesde(!openDesde)}>
                <DatePicker
                  open={openDesde}
            ...
            <ICONS.CalendarIconS
              onClick={() => setOpenDesde(!openDesde)}
              className='h-7 text-gray-600 ml-60 mt-2 z-50 absolute'
            />
          </div>
          ...
          <div className='pb-4 z-50 flex hover:cursor-pointer'>
            <div className='flex space-x-8'>
              ...
              <p onClick={() => setOpenHasta(!openHasta)}>
                <DatePicker
                  open={openHasta}
                  className='p-2 ml-1 rounded-md border-2'
                  selected={Hasta}
            ...
            <ICONS.CalendarIconS
              onClick={() => setOpenHasta(!openHasta)}
              className='h-7 text-gray-600 ml-60 mt-2 z-50 absolute'
            />
          </div>
        </div>
      ...
      <h3 className='bg-blue-500 ml-48 w-fit px-4 text-lg mb-4 -mt-14 text-white rounded-md'>
        Buscar
      </h3>
                                  ...
                                  onClick={() => {
                                    if (
                                      ...
                                      navigate('/viewrecepcion', {
                                        state: { shift: 'Nocturno' },
                                      })
                                    }
                                  }}
                                  className='text-white px-2 rounded-md bg-blue-600 hover:cursor-pointer ml-12'
                                >
                                  Ver
                                </p>
                              </div>
